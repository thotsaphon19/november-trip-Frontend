...
export default function Layout() {
  ...
  return (
    // flex (not md:flex) at every breakpoint now - on mobile this stacks
    // header above main (flex-col), on desktop it puts the sidebar beside
    // main (flex-row). Without a real flex context on mobile, <main>'s
    // height/overflow below had nothing to size against, so it just grew
    // past the screen and got clipped by this div's overflow-hidden with
    // no way to scroll to the rest of the page - that's the bug being fixed.
    <div className="h-screen overflow-hidden bg-slate-50 flex flex-col md:flex-row">
      ...
      <header className="md:hidden shrink-0 sticky top-0 z-30 bg-white border-b border-slate-200 px-4 py-3 flex items-center justify-between">
      ...
      {/* min-h-0 is the important part here - without it, a flex item
          defaults to min-height:auto (i.e. "at least as tall as my
          content"), which stops overflow-y-auto from ever kicking in and
          content just spills out past the screen instead of scrolling. */}
