...
export function usePagination(items, pageSize = 50) {
  ...
  // Resets to page 1 when the list's *content* actually changes (a new
  // search/filter reduces or grows the results) - deliberately NOT keyed on
  // the array reference itself. Every page that calls this hook rebuilds its
  // sorted/filtered array fresh on each render (`[...items].sort(...)`),
  // so a same-content array gets a brand new reference on every render -
  // including the very re-render triggered by clicking "next page". Keying
  // this effect on that reference meant every click on "next" immediately
  // bounced back to page 1. Length is a stable enough signal for "the
  // result set changed" without that false-positive.
  useEffect(() => {
    setPage(1);
  }, [items.length]);
...
/** Native <input type="date"> displays day/month/year in whatever order the
 *  browser's OS locale uses (mm/dd/yyyy on a US-locale machine, regardless
 *  of the page being in Thai) - there's no reliable cross-browser way to
 *  force an order on the native picker. This renders three plain number
 *  fields (วัน/เดือน/ปี) instead, so the order is always day-month-year no
 *  matter whose computer it's opened on. Keeps the same external contract
 *  as a native date input: `value` is a "yyyy-mm-dd" string (or ""), and
 *  onChange receives an event shaped like `{ target: { value } }` with the
 *  same format - so every existing caller keeps working unmodified.
 *  The visible year field is the Gregorian year the value already stores
 *  (not Buddhist Era) - only the field order changed, not the calendar. */
function DateInput({ value, onChange, className = "", disabled, id }) {
  ...
  // Re-sync from the outside value (e.g. switching which record is loaded)
  ...
  function emit(d, m, y) {
    ...
    const valid = complete && dNum >= 1 && dNum <= 31 && mNum >= 1 && mNum <= 12 && yNum >= 1000;
  ...
  function backspaceToPrev(e, ref) {
    if (e.key === "Backspace" && e.target.value === "") ref.current?.focus();
  }

  const segClass =
    "text-center bg-transparent focus:outline-none placeholder:text-slate-400 disabled:cursor-not-allowed disabled:opacity-60";

  return (
    <span
      className={`flex items-center gap-1 rounded-lg border border-slate-300 px-3 py-1.5 text-sm focus-within:ring-2 focus-within:ring-[var(--nt-primary)]/40 focus-within:border-[var(--nt-primary)] transition-colors ${className}`}
    >
      <input
        id={id}
        ref={dayRef}
        value={day}
        onChange={handleDay}
        onKeyDown={(e) => backspaceToPrev(e, null)}
        placeholder="วว"
        inputMode="numeric"
        disabled={disabled}
      ...
      <input
        ...
        onKeyDown={(e) => backspaceToPrev(e, dayRef)}
        placeholder="ดด"
        inputMode="numeric"
        disabled={disabled}
        className={`${segClass} w-6`}
      ...
      <input
        ...
        value={year}
        onChange={handleYear}
        onKeyDown={(e) => backspaceToPrev(e, monthRef)}
        placeholder="ปปปป"
        inputMode="numeric"
        disabled={disabled}
        className={`${segClass} w-11`}
      />
    </span>
  );
}
...
export function Input({ label, className = "", ...props }) {
  if (props.type === "date") {
    ...
    const { type, ...dateProps } = props;
    return (
      <label className="block text-sm">
        {label && <span className="block text-slate-600 mb-1">{label}</span>}
        <DateInput {...dateProps} className={className} />
      </label>
    );
  }
  return (
...
/** Same as Input but with a show/hide eye toggle - lets people check what
 *  they actually typed before submitting, which matters most exactly where
 *  a typo is costly (login, setting a new password). */
    ...
    <label className="block text-sm">
      ...
      <span className="relative block">
        <input
          ...
          className={`w-full rounded-lg border border-slate-300 pl-3 pr-9 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-[var(--nt-primary)]/40 focus:border-[var(--nt-primary)] transition-colors ${className}`}
          {...props}
        />
        <button
          type="button"
