...
import { SectionCard, Button, Input, PasswordInput, Select, Badge } from "../../components/ui";
import { Users } from "lucide-react";
import { useUndo } from "../../lib/undoContext";

const emptyUser = { email: "", password: "", confirmPassword: "", name: "", role: "STAFF" };
const emptyEditForm = { name: "", role: "STAFF", password: "", confirmPassword: "" };
...
export default function UsersTab() {
  ...
  async function addUser(e) {
    e.preventDefault();
    setError("");
    // Catch typos before they get saved - the exact scenario that locks
    // someone out with no way back in, since there's no "forgot password".
    ...
    setBusy(true);
    try {
      await api.post("/auth/users", { email: form.email, password: form.password, name: form.name, role: form.role });
      setForm(emptyUser);
      await load();
    } catch (e) {
      setError(e.message);
  ...
  function openEdit(user) {
    ...
    setEditForm({ name: user.name, role: user.role, password: "", confirmPassword: "" });
    setEditError("");
  }
  ...
  async function saveEdit(e) {
    e.preventDefault();
    setEditError("");
    // Same typo guard as addUser - this is the field that locked the last
    // person out when the retyped password silently didn't match what they
    // meant to set.
    if (editForm.password && editForm.password !== editForm.confirmPassword) {
      setEditError("รหัสผ่านและการยืนยันรหัสผ่านไม่ตรงกัน");
      return;
    }
    setEditBusy(true);
    try {
