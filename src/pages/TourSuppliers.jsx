...
import { Ticket, ArrowLeft, AlertTriangle, ArrowUp, ArrowDown, ArrowUpDown } from "lucide-react";
...
export default function TourSuppliers() {
  ...
  // dropdown, or "activities-asc"/"activities-desc" from clicking the
  // "กิจกรรม/ทัวร์" column header - same pattern as the hotel list page's
  // "สัญญา" column sort.
  const [sortBy, setSortBy] = useState("code");
  const navigate = useNavigate();
  const sortedSuppliers = (() => {
    if (sortBy === "name") {
      return [...suppliers].sort((a, b) => a.name.localeCompare(b.name, "th"));
    }
    if (sortBy === "activities-asc" || sortBy === "activities-desc") {
      const dir = sortBy === "activities-asc" ? "asc" : "desc";
      return [...suppliers].sort((a, b) => {
        const diff = (a.activities?.length || 0) - (b.activities?.length || 0);
        return dir === "asc" ? diff : -diff;
      });
    }
    return [...suppliers].sort((a, b) => a.supplierCode.localeCompare(b.supplierCode, undefined, { numeric: true }));
  })();
  const { page, setPage, totalPages, total, pageItems, rangeStart, rangeEnd } = usePagination(sortedSuppliers, 50);

  function toggleActivitiesSort() {
    setSortBy((prev) => (prev === "activities-asc" ? "activities-desc" : prev === "activities-desc" ? "code" : "activities-asc"));
  }
  ...
  async function addSupplierImage(url) {
    await api.post(`/suppliers/tours/${draftSupplier.id}/images`, { url });
    reloadDraft();
  }

  function removeSupplierImage(imageId) {
    setDraftSupplier((prev) => ({ ...prev, images: prev.images.filter((img) => img.id !== imageId) }));
    scheduleAction("ลบรูปภาพแล้ว", {
      onConfirm: () => api.delete(`/suppliers/tours/${draftSupplier.id}/images/${imageId}`),
      onUndo: () => reloadDraft(),
    });
  }
          ...
          <div className="p-5">
            {addTab === "details" && (
              <form onSubmit={saveDetails} className="space-y-3 max-w-2xl">
                ...
                <Input label="วันเวลาทำการ" placeholder="เช่น จันทร์-ศุกร์ 09:00-18:00" value={form.businessHours} onChange={(e) => setForm({ ...form, businessHours: e.target.value })} />
            ...
            {addTab === "details" && draftSupplier && (
              <div className="max-w-2xl">
                <HotelConditionsSection hotelId={draftSupplier.id} conditions={draftSupplier.conditions} onChange={reloadDraft} apiBase="/suppliers/tours" />
              </div>
            )}
            ...
            {addTab === "products" && draftSupplier && (
              <div>
                ...
                <form onSubmit={addActivity} className="space-y-2 border border-dashed border-slate-300 rounded-lg p-3">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                    <Input label="เวลาเริ่ม" type="time" value={activityForm.startTime} onChange={(e) => setActivityForm({ ...activityForm, startTime: e.target.value })} />
                    <Input label="เวลาสิ้นสุด" type="time" value={activityForm.endTime} onChange={(e) => setActivityForm({ ...activityForm, endTime: e.target.value })} />
                  </div>
                  <div>
            ...
            {addTab === "gallery" && draftSupplier && (
              <div>
                <div className="flex flex-wrap gap-2 mb-2">
                  {draftSupplier.images?.map((img) => (
                  ...
                  {draftSupplier.images?.length === 0 && <p className="text-sm text-slate-400">ยังไม่มีรูปภาพ</p>}
                </div>
      ...
      {!adding && (
        <>
          <div className="flex gap-3 mb-3 flex-wrap">
            ...
            <Select
              value={sortBy === "activities-asc" || sortBy === "activities-desc" ? "code" : sortBy}
              ...
              <table className="w-full text-sm">
                <thead className="bg-slate-50 text-slate-500 text-xs uppercase">
                  <tr>
                    ...
                    <th className="text-left px-4 py-2 min-w-[220px]">ชื่อ Supplier</th>
                    <th className="text-left px-4 py-2">จังหวัด</th>
                    <th className="text-left px-4 py-2 cursor-pointer select-none hover:text-slate-700" onClick={toggleActivitiesSort}>
                      <span className="inline-flex items-center gap-1">
                        กิจกรรม/ทัวร์
                        {sortBy === "activities-asc" && <ArrowUp size={12} />}
                        {sortBy === "activities-desc" && <ArrowDown size={12} />}
                        {sortBy !== "activities-asc" && sortBy !== "activities-desc" && <ArrowUpDown size={12} className="text-slate-300" />}
                      </span>
                    </th>
                ...
                <tbody>
                  {pageItems.map((s) => {
                    ...
                    const thumbUrl = s.logoUrl || s.activities?.find((a) => a.imageUrl)?.imageUrl;
                    const rowAlert = contractAlert(s, contractWarningDays);
                    return (
                      <tr
                        key={s.id}
                      ...
                      >
                        ...
                        <td className="px-4 py-2 font-mono text-xs whitespace-nowrap">{s.supplierCode}</td>
                        <td className="px-4 py-2">
                        ...
                        <td className="px-4 py-2 text-xs">
                          {s.contractEnd ? (
                            ...
                            >
                              {new Date(s.contractEnd).toLocaleDateString("th-TH")}
                            </span>
                          ) : (
