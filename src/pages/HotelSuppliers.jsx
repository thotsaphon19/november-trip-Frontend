...
import { Building2, ArrowLeft, AlertTriangle, ArrowUp, ArrowDown, ArrowUpDown, Copy } from "lucide-react";
import { RoomTypeRow, MealCostRow, SeasonalPricingBlock, InclusionTagPicker, MultiInputList, HotelConditionsSection, StarRatingInput, StarRatingDisplay, FacilityPicker } from "./HotelDetailPage";
...
export default function HotelSuppliers() {
  ...
  // Single sort control for the list: "code" (default) or "name" from the
  // dropdown, or "contract-asc"/"contract-desc" from clicking the "สัญญา"
  // column header - only one of these applies at a time.
  const [sortBy, setSortBy] = useState("code");
  const navigate = useNavigate();
  const sortedHotels = (() => {
    if (sortBy === "name") {
      return [...hotels].sort((a, b) => a.name.localeCompare(b.name, "th"));
    }
    if (sortBy === "contract-asc" || sortBy === "contract-desc") {
      const dir = sortBy === "contract-asc" ? "asc" : "desc";
      const withEnd = hotels.filter((h) => h.contractEnd);
      const withoutEnd = hotels.filter((h) => !h.contractEnd);
      withEnd.sort((a, b) => {
        const diff = new Date(a.contractEnd) - new Date(b.contractEnd);
        return dir === "asc" ? diff : -diff;
      });
      // Hotels with no contract end date always sort to the bottom,
      // regardless of direction - there's nothing meaningful to rank them by.
      return [...withEnd, ...withoutEnd];
    }
    // "code" (default)
    return [...hotels].sort((a, b) => a.hotelCode.localeCompare(b.hotelCode, undefined, { numeric: true }));
  })();
  ...
  function toggleContractSort() {
    setSortBy((prev) => (prev === "contract-asc" ? "contract-desc" : prev === "contract-desc" ? "code" : "contract-asc"));
  }
  ...
  async function load(query = q, province = provinceFilter, starRating = starFilter, facilityId = facilityFilter) {
  ...
  useEffect(() => {
    load(q, provinceFilter, starFilter, facilityFilter);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [q, provinceFilter, starFilter, facilityFilter]);

  useEffect(() => {
    api.get("/suppliers/hotels/facilities").then(({ data }) => setFacilityOptions(data));
  }, []);
  ...
  async function duplicateHotel(id) {
    ...
    try {
      ...
      // Jump straight into the new copy so the name/contract dates (the
      // things most likely to need changing) can be fixed right away.
          ...
          <div className="p-5">
            {addTab === "details" && (
              ...
              <form onSubmit={saveDetails} className="space-y-3 max-w-2xl">
                ...
                <FacilityPicker selectedIds={form.facilityIds} onChange={(facilityIds) => setForm({ ...form, facilityIds })} />
      ...
      {!adding && (
        <>
          ...
          <div className="flex gap-3 mb-3 flex-wrap">
            ...
            <Select value={facilityFilter} onChange={(e) => setFacilityFilter(e.target.value)} className="max-w-xs">
            ...
            <Select
              value={sortBy === "contract-asc" || sortBy === "contract-desc" ? "code" : sortBy}
              ...
              <table className="w-full text-sm">
                <thead className="bg-slate-50 text-slate-500 text-xs uppercase">
                  <tr>
                    ...
                    <th className="text-left px-4 py-2 whitespace-nowrap">รหัส</th>
                    <th className="text-left px-4 py-2 min-w-[220px]">ชื่อโรงแรม</th>
                    <th className="text-left px-4 py-2">ดาว</th>
                    <th className="text-left px-4 py-2">สิ่งอำนวยความสะดวก</th>
                    <th className="text-left px-4 py-2">จังหวัด</th>
                    <th className="text-left px-4 py-2 cursor-pointer select-none hover:text-slate-700" onClick={toggleContractSort}>
                      <span className="inline-flex items-center gap-1">
                        สัญญา
                        {sortBy === "contract-asc" && <ArrowUp size={12} />}
                        {sortBy === "contract-desc" && <ArrowDown size={12} />}
                        {sortBy !== "contract-asc" && sortBy !== "contract-desc" && <ArrowUpDown size={12} className="text-slate-300" />}
                      </span>
                    </th>
                  ...
                  {pageItems.map((h) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 py-2 font-mono text-xs whitespace-nowrap">{h.hotelCode}</td>
                        <td className="px-4 py-2">
                        ...
                        <td className="px-4 py-2">
                          {h.facilities?.length > 0 ? (
                            <div className="flex flex-wrap gap-1 max-w-[180px]">
                              {h.facilities.slice(0, 2).map((f) => (
                                <span
                                  key={f.id}
                                  className="text-[10px] whitespace-nowrap bg-slate-50 border border-slate-200 rounded-full px-1.5 py-0.5 text-slate-600"
                                >
                                  {f.label}
                                </span>
                              ))}
                              {h.facilities.length > 2 && (
                                <span className="text-[10px] text-slate-400">+{h.facilities.length - 2}</span>
                              )}
                            </div>
                          ) : (
                        ...
                        <td className="px-4 py-2 text-xs">
                          {h.contractEnd ? (
                            ...
                            >
                              {new Date(h.contractEnd).toLocaleDateString("th-TH")}
                            </span>
                          ) : (
                            <span className="text-slate-300">—</span>
                          )}
                        ...
                        <td className="px-4 py-2 text-right">
                          <div className="flex items-center justify-end gap-1.5">
                            <button
                              type="button"
                              title="ทำสำเนาโรงแรมนี้"
                              disabled={duplicatingId === h.id}
                              onClick={(e) => {
                                e.stopPropagation();
                                duplicateHotel(h.id);
                              }}
                              className="p-1.5 rounded-lg text-slate-400 hover:text-slate-700 hover:bg-slate-100 disabled:opacity-40 shrink-0"
                            >
                              <Copy size={14} />
                            </button>
                            <Button
